export-env {
  if (which simpalt | is-empty) {
    ...
    if $simpalt_version != "0.3.6" {
      print $'(ansi yellow)Prompt info:(ansi reset) Expected version (ansi white)0.3.6(ansi reset) but `simpalt` is reporting version (ansi white)'($simpalt_version)'(ansi reset)'
